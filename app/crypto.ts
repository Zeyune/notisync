...
async function developmentKey(): Promise<AESEncryptionKey> {
  // Importing is not free and the key never changes during a run.
  if (!cachedKey) {
    cachedKey = await AESEncryptionKey.import(DEVELOPMENT_KEY_BASE64, "base64");
  }
  return cachedKey;
}
...
export async function seal(value: unknown): Promise<Envelope> {
  const key = await developmentKey();
